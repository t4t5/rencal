//! Watches `~/.config/rencal/config.toml` and emits `CONFIG_CHANGED` when it
//! changes on disk, so hand-edits to the file (e.g. switching `active_group`)
//! apply live. The frontend re-fetches via RPC on the event; the file is
//! canonical, so the event carries no payload.
...
/// True if `path` points at the config file itself. We watch the whole config
/// directory (it also holds `themes/`, logs, etc.), so filter to `config.toml`.
fn is_config_file(path: &Path) -> bool {
    path.file_name() == Some(OsStr::new("config.toml"))
}

/// Watches `~/.config/rencal/` and emits `CONFIG_CHANGED` when `config.toml` changes.
...
mod tests {
    ...
    fn matches_config_toml_only() {
        assert!(is_config_file(Path::new("/home/u/.config/rencal/config.toml")));
