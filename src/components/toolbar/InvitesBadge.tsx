...
import { listInvites, rsvp } from "@/lib/api/calendar-events"
...
export function InvitesBadge() {
  ...
  useEffect(() => {
    ...
    listInvites(slugs).then(setPendingInvites).catch(console.error)
  }, [calendars])
  ...
  const handleRsvp = async (invite: CalendarEvent, response: ResponseStatus) => {
    ...
    try {
      await rsvp(invite.calendar_slug, invite.id, response)
      void requestSync()
    } catch (e) {
